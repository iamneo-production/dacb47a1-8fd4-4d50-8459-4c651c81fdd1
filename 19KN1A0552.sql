...
SELECT * FROM ELECTION WHERE ST_NAME = 'Bihar' AND PARTYNAME='BJP';
    ...
    AS TOTAL_CANDIDATES 
        FROM ELECTION 
         WHERE ST_NAME ='West Bengal' AND YEAR=2014              /* for this query data is not fetched because */
           GROUP BY ST_NAME 
            ORDER BY YEAR ;                    /*there is no data given about west bengal in the table*/
...
/* write a sql query to find the Total vote percentage of BJP get in goa state in the year 2003 */
 SELECT ST_NAME,COUNT(TOTVOTPOLL) AS TOT_VOTE_PERCENTAGE                 
    FROM ELECTION 
        WHERE ST_NAME='Goa' AND YEAR=2003           /* no data given about state goa not given in the table*/     GROUP BY ST_NAME                                        
          ORDER BY YEAR ;

    /* write a sql query to find How many times has BJP gotten an Above 50% votes */
       SELECT COUNT(TOTVOTPOLL) AS VOTE_PERCENTAGE 
        FROM ELECTION 
          WHERE PARTYNAME='BJP' AND TOTVOTPOLL > 50 ;
